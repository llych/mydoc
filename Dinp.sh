...
echo "Registry"
docker run --name registry -d -v /root/dinp/data/registry:/tmp/registry -p 5000:5000 --restart=always registry
...
echo "Mysql"
docker run --name mysql -v /root/dinp/data/mysql:/var/lib/mysql -P -e MYSQL_ROOT_PASSWORD=toor -d --restart=always mysql
